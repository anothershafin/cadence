...
(function () {
  ...
  function fmt12short(t){ let [h,m]=t.split(':').map(Number); const ap=h>=12?'p':'a'; h=h%12||12; return m===0?`${h}${ap}`:`${h}:${pad(m)}${ap}`; }
  function prettyDate(ds){
    const [y,mo,d]=ds.split('-').map(Number); const dt=new Date(y,mo-1,d);
    const t=todayStr();
  ...
  function relDays(ds){ const a=new Date(todayStr()), b=new Date(ds); return Math.round((a-b)/86400000); }

  /* ---------- seed routine (from uploaded timetable) ---------- */
  function seedEvents(){
    // course -> color + faculty/section meta
    const C = {
    ...
    // [course, day, start, end, block, room]
    const R = [
  ...
  function renderToday(){
    const d = new Date();
    const dow = d.getDay();
    $('#today-eyebrow').textContent = DAYS[dow];
    ...
    const todays = db.events.filter(e=>e.day===dow).sort((a,b)=>toMin(a.start)-toMin(b.start));
    $('#today-greeting').textContent = `${greet} · ${todays.length} class${todays.length!==1?'es':''} scheduled`;
  ...
  function renderFocus(todays){
    const card = $('#focus-card');
    const nm = nowMin();
    const current = todays.find(e=>nm>=toMin(e.start) && nm<toMin(e.end));
    const next = todays.find(e=>toMin(e.start)>nm);
    ...
    if(current){
      ...
      card.innerHTML = `
        ...
        <p class="meta"><span>${fmt12(current.start)} – ${fmt12(current.end)}</span><b>${esc(eventSubtitle(current))}</b></p>`;
    } else if(next){
      ...
      card.innerHTML = `
        ...
        <p class="meta"><span>${fmt12(next.start)} – ${fmt12(next.end)}</span><b>${esc(eventSubtitle(next))}</b></p>`;
    } else {
      // no class now/next -> show task progress
      ...
      const msg = todays.length===0 ? 'No classes today' : 'Classes done for today';
  ...
  function renderTimeline(todays){
    const wrap = $('#timeline');
    $('#day-count').textContent = todays.length ? `${todays.length} event${todays.length>1?'s':''}` : '';
    wrap.innerHTML='';
    if(todays.length===0){
      wrap.appendChild(el('div','empty','<div class="em">○</div><p>Nothing scheduled today.</p><p class="hint">Enjoy the open day, or add an event in Routine.</p>'));
      return;
    }
    const nm = nowMin();
    ...
    todays.forEach(e=>{
      ...
      // insert now-line before the first event that hasn't started yet
      if(!nowInserted && nm < s && !(nm>=s&&nm<en)){
        wrap.appendChild(nowLine(nm)); nowInserted=true;
      }
      ...
      item.innerHTML = `
        ...
        <div class="tl-card" data-edit-event="${e.id}">
          ...
          <div class="tl-time"><span class="s tnum">${fmt12short(e.start)}</span><span class="e tnum">${fmt12short(e.end)}</span></div>
          <div class="tl-body"><div class="t">${esc(eventName(e))}${state==='now'?' · now':''}</div><div class="d">${esc(eventSubtitle(e))}</div></div>
        </div>`;
      wrap.appendChild(item);
    });
    if(!nowInserted){ wrap.appendChild(nowLine(nm)); } // now is after all events
  }
  function nowLine(nm){
    const h=Math.floor(nm/60), m=nm%60;
    const d=new Date(); d.setHours(h,m);
    const line=el('div','now-line');
    line.innerHTML=`<div class="nl-rail"><div class="nl-pin"></div></div><div class="nl-bar"></div><div class="nl-time tnum">Now ${fmt12short(pad(h)+':'+pad(m))}</div>`;
    return line;
  }

  function todayTodos(){ const t=todayStr(); return db.todos.filter(td=>td.date===t); }
  function renderTodayTasks(){
    const wrap=$('#today-tasks');
    const list=todayTodos().sort(sortTodos);
    const active=list.filter(t=>!t.done), done=list.filter(t=>t.done);
    $('#today-task-count').textContent = list.length ? `${done.length}/${list.length} done` : '';
    wrap.innerHTML='';
    if(list.length===0){
      wrap.appendChild(el('div','empty','<div class="em">✓</div><p>No tasks yet for today.</p><p class="hint">Anything left from earlier days lands here automatically.</p>'));
      return;
    }
    active.forEach(t=>wrap.appendChild(taskRow(t)));
  ...
  function renderRoutine(){
    ...
    DAYS_SHORT.forEach((dn,i)=>{
      const cnt=db.events.filter(e=>e.day===i).length;
    ...
    const list=db.events.filter(e=>e.day===routineDay).sort((a,b)=>toMin(a.start)-toMin(b.start));
    $('#routine-count').textContent=list.length?`${list.length} event${list.length>1?'s':''}`:'';
    const wrap=$('#routine-list'); wrap.innerHTML='';
    if(list.length===0){
      wrap.appendChild(el('div','empty',`<div class="em">◇</div><p>No events on ${DAYS[routineDay]}.</p><p class="hint">Tap + to add one.</p>`));
      return;
    }
    list.forEach(e=>{
      const col=COLORS[e.color]||COLORS.slate;
      ...
      row.innerHTML=`
        ...
        <div class="rev-time"><span class="s tnum">${fmt12short(e.start)}</span><span class="e tnum">${fmt12short(e.end)}</span></div>
  ...
  function renderInsights(){
    ...
    if(!compDays.has(dateStrOf(cur))) cur.setDate(cur.getDate()-1); // allow today not-yet
    while(compDays.has(dateStrOf(cur))){ streak++; cur.setDate(cur.getDate()-1); }
    // weekly classes + busiest day
    const perDay=DAYS.map((_,i)=>db.events.filter(e=>e.day===i).length);
    ...
    drawBars('#barchart', days.map((ds,i)=>({
      label:DAYS_SHORT[new Date(ds).getDay()][0], value:counts[i], today:ds===todayStr()
    })));

    // weekly load (classes per weekday)
    drawBars('#loadchart', DAYS_SHORT.map((dn,i)=>({
      label:dn[0], value:perDay[i], today:i===new Date().getDay()
    })));
  }
  ...
  /* ===========================================================
     ACTIONS
     =========================================================== */
  function addTodo(text, prio, note){
    text=text.trim(); if(!text) return;
    db.todos.push({ id:uid(), text, date:todayStr(), done:false, prio:!!prio, note:note||'', createdAt:Date.now(), carried:0 });
    save(); refresh();
  }
  ...
  function taskSheet(id){
    ...
    $('#f-save').onclick=()=>{
      ...
      else { db.todos.push({id:uid(),text,note,prio,date:todayStr(),done:false,createdAt:Date.now(),carried:0}); }
  ...
  function eventSheet(id){
    ...
    openSheet(`
      ...
      <div class="field"><label>${e?'Day':'Repeats on'}</label>
    ...
    let days=new Set(sel), color=e?e.color:'teal';
    $('#e-days').querySelectorAll('button').forEach(b=>b.onclick=()=>{
      const i=+b.dataset.d;
      if(e){ days=new Set([i]); $('#e-days').querySelectorAll('button').forEach(x=>x.classList.remove('on')); b.classList.add('on'); }
      else { if(days.has(i)){days.delete(i);b.classList.remove('on');} else {days.add(i);b.classList.add('on');} }
    });
    ...
    $('#e-save').onclick=()=>{
      ...
      const base={ title, course:title, section:$('#e-section').value.trim(), faculty:$('#e-faculty').value.trim(),
                   block:$('#e-block').value.trim(), room:$('#e-room').value.trim(), start, end, color,
                   type: e? e.type : (/^[A-Z]{2,4}\d{3}/.test(title)?'class':'custom') };
      if(e){ Object.assign(e, base, {day:[...days][0]}); }
      else { [...days].forEach(d=> db.events.push(Object.assign({id:uid(),day:d},base))); }
      save();
      routineDay=[...days][0];
      refresh(); closeSheet();
    };
  ...
  function init(){
    ...
    // live clock — refresh Today every 30s so now/next + timeline stay current
    setInterval(()=>{ if(tab==='today') renderToday(); }, 30000);
    // re-check rollover + theme when app regains focus (e.g. next morning)
    document.addEventListener('visibilitychange',()=>{ if(!document.hidden){ rollover(); applyTheme(); refresh(); } });
    matchMedia('(prefers-color-scheme:dark)').addEventListener('change',()=>{ if(db.meta.theme==='auto') applyTheme(); });

    // service worker (only works when hosted over http/https)
